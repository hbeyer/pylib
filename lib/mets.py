...
class METSBuilder:
        def __init__(self, sign, title="-", folder = "mss"):
                self.title = title
                self.folder = folder
                self.sign = sign
                self.url = "http://diglib.hab.de/" + self.folder + "/" + sign + "/start.htm"
                self.struct = []
                try:
                        ...
                        try:
                                facs = ul.urlopen("http://diglib.hab.de/" + self.folder + "/" + self.sign + "/facsimile.xml")
                        except:
                                print("Keine facsimile.xml vorhanden unter http://diglib.hab.de/" + self.folder + "/" + self.sign + "/facsimile.xml")
                                self.countPages()
                        else:
                                self.facs = facs
                                self.getStruct()
        ...
        def countPages(self):
                ...
                while 1 == 1:
                        page = str(i).zfill(5)
                        url = "http://diglib.hab.de/" + self.folder + "/" + self.sign + "/min/" + page + ".jpg"
                        try:
                                fo = ul.urlopen(url)
                        except:
                                return(1)
        ...
        def build(self):
                ...
                for fg in fgg:
                        fg = self.appendFiles(fg, self.struct, self.sign)
        ...
        def appendFiles(self, fg, struct, sign):
                use = fg.attrib["USE"]
                ...
                for struct in struct:
                        ...
                        flocatEl.set("LOCTYPE", "URL")
                        flocatEl.set("{http://www.w3.org/1999/xlink}href", "http://diglib.hab.de/" + self.folder + "/" + sign + concordance[use] + "/" + struct["page"] + ending)
                return(fg)

def extractNumber(number):
        no = re.findall("/([0-9]+).jpg", number)
        try:
                return(no[0])
        except:
                print("Editionsnummer kann nicht gefunden werden")
                return(None)
